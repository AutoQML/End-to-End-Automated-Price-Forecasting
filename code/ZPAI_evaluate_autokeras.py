...
def evaluate_autokeras(X_train: pd.DataFrame, 
    ...
    print('\n --AutoKeras approach-- for feature set: {}'.format(feature_set))
    ...
    f.write('\n\n')
    f.write('Feature set for AutoKeras: \t' + feature_set)
    f.write('\n')
   ...
   # flaml built-in metric: https://microsoft.github.io/FLAML/docs/Use-Cases/Task-Oriented-AutoML#customize-automlfit
    ...
    filename = "{}-{}-{}-{}.{}".format(M_DATE,input_filename,'autokeras-test-result',feature_set,'png')
    title = str("Autokeras results for {} on feat. set {}".format(input_filename, feature_set))
    plt.title(title)
